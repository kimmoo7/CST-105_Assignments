...
public class NumberThirteen {
	...
	public static void main(String[] args) {
		...
		else if (status == 1) { // Compute for married filing jointly or qualifying widow
			if (income <= 16700)
				tax = income * 0.10;
		...
		else if (status == 2) { // Compute for married filing separately 
			if (income <= 8350)
				tax = income * 0.10;
			else if (income <= 33950)
				tax = 8350 * 0.10 + (income - 8350) * 0.15;
		...
		else if (status == 3) { // Compute for head of household
			if (income <= 11950)
				tax = income * 0.10;
